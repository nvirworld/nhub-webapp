...
const BridgeView: React.FC = () => {
  ...
  const { data: bridges, isLoading, isError } = useQueryBridges(myWalletData ?? null)
  ...
  useEffect(() => {
    if (bridges != null) {
      for (let i = 0; i < bridges.length; i++) {
        if (bridges[i].inToken.network.chainId === myWalletData?.networkChainId) {
          setSelectedIdx(i)
          break
        }
      }
  ...
  const bridgeTransfer = useMutationBridgeTransfer(onSuccessTransfer, onFailTransfer)
  ...
  return <Content.Layout id="bridge">
    <Content.Header>
          ...
          {bridge != null ?
            <>
              ...
              <Content.ToNetworkWrapper span={24} style={{ marginTop: 30 }}>
                ...
                {bridge.outToken.network.iconUrl != null ? <img width="24" height="24" src={bridge.outToken.network.iconUrl} /> : <></>}
                <Shared.NhubTypo type='regular' usage='bridge'>
